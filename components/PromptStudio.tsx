...
export default function PromptStudio({ thoughts, selectedThoughts }: PromptStudioProps) {
  ...
  // Get the selected thoughts data
  const selectedThoughtData = thoughts.filter(t => selectedThoughts.includes(t.id));
  ...
  const enhancePrompt = useCallback(async () => {
    ...
    try {
      // Create context from selected thoughts
      const context = selectedThoughtData.length > 0 
        ...
        : thoughts
            .filter(t => t.status === 'active' && t.atp > 50)
            .slice(0, 5)
            .map(t => ({
              text: t.text,
              tags: t.tags,
...
Return only the enhanced prompt, no explanation.`,
      ...
      const contextText = selectedThoughtData.length > 0
        ? selectedThoughtData.map(t => t.text).join('. ')
  ...
  }, [originalPrompt, selectedThoughtData, thoughts]);
      ...
      <div className="flex-1 p-4 space-y-4 overflow-y-auto">
        ...
        {selectedThoughts.length > 0 && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <h3 className="text-sm font-medium text-blue-900 mb-2">
              Selected Context ({selectedThoughts.length} thoughts)
            </h3>
            <div className="space-y-2 max-h-32 overflow-y-auto">
              {selectedThoughtData.map(thought => (
                <div key={thought.id} className="text-xs text-blue-800 bg-blue-100 rounded p-2">
                  <div className="font-medium">{thought.text.slice(0, 100)}...</div>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-blue-600">ATP: {thought.atp.toFixed(0)}</span>
                    {thought.tags.length > 0 && (
                      <span className="text-blue-600">
                        Tags: {thought.tags.slice(0, 3).join(', ')}
                      </span>
                    )}
                  </div>
                </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full min-h-32 p-4 border border-gray-300 rounded-lg resize-y focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm"
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
          ...
          <ol className="text-xs text-gray-600 space-y-1">
            <li>1. Select thoughts from the graph (optional - uses active thoughts if none selected)</li>
            <li>2. Enter your original prompt</li>
            <li>3. Click "Enhance Prompt" to add context from your thoughts</li>
            <li>4. Copy the enhanced prompt for use elsewhere</li>
          </ol>
        </div>
      </div>
    </div>
